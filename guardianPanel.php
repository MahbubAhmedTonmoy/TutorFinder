...
<div class="sidenav">
        ...
        <select class="form-control" name="postID">
                        ...
                        $un = $_SESSION['un'];
                        //$un = "01698456325";
                        $sql = "SELECT post_id FROM g_post WHERE g_id = $un";
    ...
    if(isset($_POST['submit'])){
            ...
            $sql = "SELECT post_id, g_id, subjects, medium, salary, divisions, address, dateTime FROM g_post WHERE g_id = $un and post_id = $postID";
            $rows = mysqli_query($conn, $sql) or die("database error:". mysqli_error($conn));			
		    while($row = mysqli_fetch_assoc($rows)){ 
    ?>
            <div class="title">
                <a href="#"><b>Post Id: </b><?php echo $row['post_id']; ?></a>
					...
					<div class="desc"><b>Your Id: </b> <a target="_blank" href="<?php echo "gProfile.php?id=$row[g_id]"; ?>"><?php echo $row['g_id']; ?></a></div>		
...
<div class="row">
            ...
            $sql = "SELECT a.id, a.fullname, a.institutionname, a.phone, a.email, a.nid, a.address, b.post_id FROM tutor_info a, apply_g_post b where a.phone = b.tutor_id and b.post_id =$idpost and b.g_id =$un";
			$rows = mysqli_query($conn, $sql) or die("database error:". mysqli_error($conn));			
			while($row = mysqli_fetch_assoc($rows)){
            ?>
        <div class="col-md-<?php echo $bootstrapColWidth; ?>">    
            <div class="card hovercard">
                ...
                <div class="card-body info">
					...
					<div class="desc"><b>Tutor Id: </b> <a target="_blank" href="<?php echo "tProfile.php?id=$row[id]"; ?>"><?php echo $row['id']; ?></a></div>		
                    <div class="desc"><b>Name: </b><?php echo $row['fullname']; ?></div>      
                    <div class="desc"><b>Institution: </b><?php echo $row['institutionname']; ?></div> 
