 ...
 <title>Profile Information</title>
  <div class="header">
  	<h2>Tutor Finder Profile Information</h2>
		...
		<?php 
			 if($type == "Student"){
				$sql = "SELECT subjects, medium, salary, divisions, address from st_post where st_id = $user";
			}
			else if($type == "Gaurdian"){
				$sql = "SELECT subjects, medium, salary, divisions, address from g_post where g_id = $user";
			}
			else if($type == "Tutor"){
				$sql = "SELECT subjects, medium, salary, divisions, address from tutor_post where tutor_id = $user";
			}
			$resultset = mysqli_query($conn, $sql) or die("database error:". mysqli_error($conn));
			while( $record = mysqli_fetch_array($resultset) ) {
		?>
<form  name="index" method="post" action="editposthandler.php" onsubmit="return(validate());">	
    ...
    <div class="input-group">
  	  <label>Division</label>
			<input type="text" name="divisions" value="<?php echo $record['divisions']; ?>">
			<p id="ferror" style="color:red;"></p>
  	</div>
	
	<div class="input-group">
  	  <label>Medium</label>
			<input type="text" name="medium" value="<?php echo $record['medium']; ?>">
	...
	<div class="input-group">
  	  <label>Subject</label>
			<input type="text" name="subject" value="<?php echo $record['subjects']; ?>">
			<p id="ferror" style="color:red;"></p>
  	</div>
